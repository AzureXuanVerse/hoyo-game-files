...
<template>
  <div class="relative">
    <button
      class="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
      @click="toggle"
    >
      <slot name="trigger" :label="selectedLabel" :value="modelValue">
        {{ selectedLabel }}
      </slot>
      <LucideChevronDown class="h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform" :class="show ? 'rotate-180' : ''" />
    </button>
